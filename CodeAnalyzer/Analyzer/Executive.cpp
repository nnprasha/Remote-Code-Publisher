...
//Find category for the specified file
std::string CodeAnalysisExecutive::findCategory(std::string& fileSpec)
{
	std::string cat = "";

	if (fileSpec.find("Category1") != -1)
		cat = "Category1\\";
	else if (fileSpec.find("Category2") != -1)
		cat = "Category2\\";
	else
		cat = "Category3\\";
...
//Analyzing all files and calling code publisher for each one of them
NoSqlDB<std::string> CodeAnalysisExecutive::FileAnalysis(std::vector<std::string> a)
{
	TypeTable tt;
	ScopeStack<ElementType> s;
	CodePublisher cp(scriptingFilesPath,webPageRepoPath,htmlHeader,startPageHeader,a);

	//Constructing Type Table for DepAnalysis
	std::cout << "\n--> Constructing Type Table..";
	tt.ASTdfs(Repository::getInstance()->getGlobalScope(), s);
	std::cout << "\n--> TypeTable construction completed!!";	
	DepAnal da(tt);
	std::string startUpPageTableRows = "";
	std::vector<std::string> filesToCopy;
	std::string cat = "";
	std::cout << "\n--> Dependency Analysis Started...(REQUIREMENT 7)";
	for (auto file : cppHeaderFiles())
	{
		db=da.dependencyAnalsis(file);
		filesToCopy.push_back(file);
		std::string fileFullSpec = FileSystem::Path::getFullFileSpec(file);		
		cat = findCategory(fileFullSpec);
		startUpPageTableRows += "<tr>\n<td><a href=\"../" + cat+FileSystem::Path::getName(file, true) + ".htm" + "\">" +
		FileSystem::Path::getName(file, true) + "</td>\n<td>" + FileSystem::Path::getPath(file) + "</td>\n</tr>\n";
	}
	for (auto file : cppImplemFiles())
	{
		db = da.dependencyAnalsis(file);
	...
	//Calling Code publisher for all the files to publish
	cp.copytoHtmlFiles(filesToCopy, db);
	cp.buildStartUpPage(startUpPageTableRows,a,cat);
	//cp.openStartUpPage();
	return db;
}

//Extracting from cmd line
void CodeAnalysisExecutive::ExtractFromCommandLine(int argc, char* argv[])
{
	for (int i = 1; i < argc; i++)
	{
		std::string path = std::string(argv[i]);
		if (path == "/ScriptingPath")
			scriptingFilesPath = argv[i + 1];
		else if (path == "/WebPageRepo")
			webPageRepoPath = argv[i + 1];
		else if (path == "/HtmlHeader")
			htmlHeader = argv[i + 1];
		else if (path == "/StartPageHeader")
			startPageHeader = argv[i + 1];
	}
	if (FileSystem::File::exists(scriptingFilesPath + htmlHeader))
		FileSystem::File::remove(scriptingFilesPath + htmlHeader);

	if (FileSystem::File::exists(scriptingFilesPath + startPageHeader))
		FileSystem::File::remove(scriptingFilesPath + startPageHeader);

}

NoSqlDB<std::string> CodeAnalysisExecutive::DemoReq9(std::vector<std::string> a)
{

	std::cout << "--> Starting the CodePublisher...";
	
	return FileAnalysis(a);
}


//Copying important project files for req 9
std::vector<std::string> CodeAnalysisExecutive::copyImportantProjectFiles(char* argv[])
{
	std::vector<std::string> impPackages = { "CodePublisher","DepAnal","TypeAnalyzer","Parser","Tokenizer","AbstractSyntaxTree","FileSystem" };
	
	std::string prefixPath = "../";
	std::string impDirPath = argv[1];
	std::cout << "\n-->Prior to Requirement Display, we need to copy .h and .cpp files to the \n ImportantPackages Folder for analysis!!";
	for (std::string package : impPackages)
	{
		std::cout << "\n\n\t--> Important Package: " << package <<". Files that are being copied: ";		
		
		std::vector<std::string> hFiles = FileSystem::Directory::getFiles(prefixPath + package, "*.h");
		
		for (auto file : hFiles)
		{
			//Copying .h files
			std::string src = FileSystem::Path::getFullFileSpec(prefixPath + package + "/" + file);
			std::string dest = FileSystem::Path::getFullFileSpec(impDirPath + FileSystem::Path::getName(file));
			bool x=FileSystem::File::copy(src,dest,false);	
			std::cout << "\n\t\t-->" << file;
		}

		std::vector<std::string> cppFiles = FileSystem::Directory::getFiles(prefixPath + package, "*.cpp");
	...
	std::cout << "\n--> All Files have been copied to the "<<argv[1]<<" Folder!!";
	return impPackages;
}

#ifdef TESTING_EXECUTIVE
