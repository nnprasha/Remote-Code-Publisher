...
* 
* void DemoReq9()
*	-Calls code publisher
*
* void fileAnalysis()
*	-Retrieves files and performs DepAnalysis by calling DepAnal, and then calls code Publisher. 
*
* void ExtractFromCommandLine(int argc,char* argv[]);
*	-Extracts arguments from cmd line and passes them to the code publisher. 
*
* copyImportantProjectFiles();
*	-Copies important packes to a folder (path from cmd line) to process these files for Code Publishing
*
*  Required Files:
*  ---------------
...
*  Maintanence History:
*  --------------------
*  ver 1.5 : 2nd May 2017
*	- Added functionality to implement remote code publisher
*
*  ver 1.4 : 4th April 2017
*  - Added functionality to implement Code Publisher
*
*  ver 1.3 : 28 Oct 2016
...
namespace CodeAnalysis
{
  ...
  public:
	...
	void InitialAnalysis(int argc, char* argv[], CodeAnalysisExecutive& exec);
	NoSqlDB<std::string> FileAnalysis(std::vector<std::string> a);
	
	void ExtractFromCommandLine(int argc,char* argv[]);
	std::string findCategory(std::string& fileSpec);
	NoSqlDB<std::string> DemoReq9(std::vector<std::string> a);
	
	std::vector<std::string> copyImportantProjectFiles(char* argv[]);
