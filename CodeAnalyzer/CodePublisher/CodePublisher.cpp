	...
	NoSqlDB<std::string>& db, std::vector<std::string> deps)
{
	...
	std::FILE* myfile;
	if (file.find("Category1") != -1)
		cTime = "Category1";
	else if (file.find("Category2") != -1)
		cTime = "Category2";
	else
		cTime = "Category3";
...
void CodePublisher::buildStartUpPage(const std::string& rows,std::vector<std::string> a,std::string& cat)
{
	...
	std::FILE* myfile;
	std::string filePath = webPageRepoPath + "/" + cat + startUpPageName;
